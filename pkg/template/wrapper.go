...
var PythonWrapper = `#!/usr/bin/env python3
...
def term_all_children():
    ...
    if all_exited:
        ...
        for pid in pid_list:
            logger.warning("child PID={} did not exit cleanly, killing".format(pid))
...
def fork_and_exec():
    ...
    global epoch_by_base_id

    args = sys.argv[1].split()
    base_id = None
    for i, arg in enumerate(args):
        if arg == "--base-id" and i + 1 < len(args):
            base_id = args[i + 1]
            break
        elif arg.startswith("--base-id="):
            base_id = arg.split("=")[1]
            break

    if not base_id:
        logger.error("Missing required --base-id argument in: {}".format(sys.argv[1]))
        sys.exit(1)

    if base_id not in epoch_by_base_id:
        epoch_by_base_id[base_id] = 0

    epoch = epoch_by_base_id[base_id]
    epoch_by_base_id[base_id] += 1

    logger.info("forking and execing new child process at epoch {} for base-id {}".format(epoch, base_id))

    # remove old restart-epoch if exists
    args = [arg for arg in args if not arg.startswith("--restart-epoch")]
    args.extend(["--restart-epoch", str(epoch)])
...
def main():
    ...
    if len(sys.argv) < 2:
        logger.error("Usage: {} <envoy command string>".format(sys.argv[0]))
        sys.exit(1)
