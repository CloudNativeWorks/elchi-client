...
func (c *Client) Connect(ctx context.Context) error {
	...
	if success {
		// Connection successful, start monitor
...
func (c *Client) getConnectionParams() *connectionParams {
	return &connectionParams{
		connectTimeout:   30 * time.Second, // Bağlantı zaman aşımı
		keepaliveTime:    1 * time.Minute,  // Her dakika ping at
		keepaliveTimeout: 20 * time.Second, // Ping timeout
	}
}
