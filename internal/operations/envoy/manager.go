...
func (m *Manager) ProcessEnvoyVersionCommand(request *client.RequestEnvoyVersion) *client.ResponseEnvoyVersion {
	response := &client.ResponseEnvoyVersion{
		Status:              client.EnvoyVersionStatus_SUCCESS,
	...
	case client.EnvoyVersionOperation_GET_VERSIONS:
		...
		if err != nil {
			...
			response.Status = client.EnvoyVersionStatus_DIRECTORY_ERROR
	...
	case client.EnvoyVersionOperation_SET_VERSION:
		if request.Version == "" {
			...
			response.Status = client.EnvoyVersionStatus_VERSION_NOT_FOUND
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "not found in archive") {
				...
				response.Status = client.EnvoyVersionStatus_DOWNLOAD_FAILED
			} else if strings.Contains(err.Error(), "network") || strings.Contains(err.Error(), "connection") {
				response.Status = client.EnvoyVersionStatus_NETWORK_ERROR
			} else if strings.Contains(err.Error(), "permission") {
				response.Status = client.EnvoyVersionStatus_PERMISSION_FAILED
			} else {
				response.Status = client.EnvoyVersionStatus_DOWNLOAD_FAILED
			}
			response.ErrorMessage = err.Error()
