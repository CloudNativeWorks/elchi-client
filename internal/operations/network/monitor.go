...
func (cm *ConnectionMonitor) MonitorConnectionDuringApply(ctx context.Context) bool {
	...
	cm.logger.Debugf("gRPC connection available: %t, Client ID: %s", cm.grpcConn != nil, cm.clientID)
...
func (cm *ConnectionMonitor) checkConnectivity() bool {
	...
	if cm.grpcConn != nil {
		...
		cm.logger.Debug("No gRPC connection available, skipping gRPC ping")
	}
...
// grpcPingCheck performs gRPC ping check using the Ping RPC
func (cm *ConnectionMonitor) grpcPingCheck() bool {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()

	pingClient := client.NewCommandServiceClient(cm.grpcConn)
