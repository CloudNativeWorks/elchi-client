...
# Check if sudoers file needs update
SUDOERS_UPDATED=false
if [[ -f "$SUDO_FILE" ]]; then
  # Check if file contains the new netplan rules
  if ! grep -q "/etc/netplan/90-\*.yaml" "$SUDO_FILE" 2>/dev/null; then
    info "📝 Updating existing sudoers file with new rules"
    SUDOERS_UPDATED=true
  else
    info "✅ Sudoers file already up-to-date"
  fi
else
  info "📝 Creating new sudoers file"
  SUDOERS_UPDATED=true
fi
